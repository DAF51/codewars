...
function findEvenIndex(arr){
  let ind = arr.findIndex((val,i) => {
    let leftSide = arr.filter((current,ind) => {return ind < i})
    let rightSide = arr.filter((current,ind) => {return ind > i})
    let leftSideVal = leftSide.reduce((a,b) => a+b, 0)
    let rightSideVal = rightSide.reduce((a,b) => a+b,0)
