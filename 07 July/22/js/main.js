...
// arrangements, the wave can instead reflect back and forth through the crowd. When the gap in seating is narrow, the 
// wave can sometimes pass through it. Usually only one wave crest will be present at any given time in an arena, although 
...
function wave(str){
  ...
  strArr.forEach((val, ind) =>{
    ...
    if(val){
      val = val.toUpperCase()
      waveArr.push(strCopy.slice(0,ind)+val+strCopy.slice(ind+1))
    } 
  })
  waveArr = waveArr.filter(val => {
    return val != str
  })
  return waveArr
}
