...
// (that is, only the first letter of the word is in upper case) or (b) considered to be an exception and put 
...
function titleCase(title, minorWords) {
  ...
  let titleArr = title? title.toLowerCase().split(" ") : ""
  let minorArr = minorWords? minorWords.toLowerCase().split(" ") :""
  let titleCased

  if(titleArr && minorArr){
    titleCased = titleArr.map((word,i) => {
  ...
  } else if(titleArr && !minorArr){
    titleCased = titleArr.map(word => {
      return word[0].toUpperCase() + word.slice(1)
    })
  } else {
    return ""
...
// console.log(titleCase("","One Two Three"))
