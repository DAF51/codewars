...
function solve(s) {
  let wordSplit= s.split(/[aeiou]/)
  let pointVal = []
  wordSplit.forEach((sub) => {
    if(sub.length === 1){
      return pointVal.push(sub.charCodeAt(0)-96)
    } else{
        let subSplit=sub.split("")
        let vals = subSplit.map((letter) => letter.charCodeAt(0)-96)
        return pointVal.push(vals.reduce((a,b)=> a+b,0))
    }
  })
  return Math.max(...pointVal)
}
