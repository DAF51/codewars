...
// array/list. If there are multiple elements with the same value, remove the one 
// with a lower index. If you get an empty array/list, return an empty array/list.
...
function removeSmallest(numbers) {
  const smallArr = numbers.slice().sort((a,b)=> a-b)
  const smallest = smallArr.shift()
  console.log(numbers.indexOf(smallest))
  const removed = numbers.slice()
  removed.splice(numbers.indexOf(smallest),1)
  return removed
}
